...
export class BunnyStorageAdapter implements GeneratedAdapter {
  ...
  handleUpload: HandleUpload = async ({ data, file }) => {
  ...
  generateURL: GenerateURL = ({ filename, prefix = "" }) => {
  ...
  staticHandler: StaticHandler = async (req, res, next) => {
    try {
      ...
      const url = `https://storage.bunnycdn.com/${
        this.config.zone
...
export interface BunnyAdapterConfig {
   ...
   * @example "myzone" - resolves to https://storage.bunnycdn.com/myzone
   */
  zone: string;
  /**
...
const bunnyStorage = ({
  ...
  return ({ collection, prefix }) => {
    return new BunnyStorageAdapter(
      { zone, accessKey, pullZone, useZoneInUrl },
      { collection, prefix }
    );
  };
};
